...
public class AuthenticationActivity extends Activity {
	...
	// one active activity
	private class ActivityBroadcastReceiver extends
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equalsIgnoreCase(
				...
				try {
					...
					Logger.e(TAG,
							"ActivityBroadcastReceiver onReceive exception",
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Logger.v(
				TAG,
				"Init broadcastReceiver with requestId:"
	...
	protected void onResume() {
		...
		if (mRestartWebview) {
			Logger.v(TAG, "Webview onResume will post start url again:"
					+ mStartUrl);
			final String postUrl = mStartUrl;

			if (mReceiver != null) {
				Logger.v(TAG,
						"Webview onResume register broadcast receiver for requestId"
	...
	private class CustomWebViewClient extends WebViewClient {
		...
		public void onReceivedError(WebView view, int errorCode,
			...
			displaySpinner(false);
			Logger.e(TAG, "Webview received an error. Errorcode:" + errorCode
					+ " " + description, "", ADALError.ERROR_WEBVIEW);
			Intent resultIntent = new Intent();
			resultIntent.putExtra(
					AuthenticationConstants.Browser.RESPONSE_ERROR_CODE,
		...
		public void onReceivedSslError(WebView view, SslErrorHandler handler,
			...
			displaySpinner(false);
			handler.cancel();
			Logger.e(TAG, "Received ssl error", "",
					ADALError.ERROR_FAILED_SSL_HANDSHAKE);
