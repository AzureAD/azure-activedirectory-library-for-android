...
class Oauth2 {
	...
	public String getTokenEndpoint() {
		return mRequest.getAuthority()
				+ AuthenticationSettings.INSTANCE.getTokenEndpoint();
	}
	...
	public String getCodeRequestUrl() throws UnsupportedEncodingException {
		...
		if (!StringExtensions.IsNullOrBlank(mRequest
				.getExtraQueryParamsAuthentication())) {
	...
	public String buildTokenRequestMessage(String code)
		...
		String message = String
				.format("%s=%s&%s=%s&%s=%s&%s=%s",
						...
						StringExtensions
								.URLFormEncode(AuthenticationConstants.OAuth2.AUTHORIZATION_CODE),

						AuthenticationConstants.OAuth2.CODE, StringExtensions
								.URLFormEncode(code),
						...
						StringExtensions.URLFormEncode(mRequest
								.getRedirectUri()));
	...
	public static AuthenticationResult processUIResponseParams(
		...
		if (response.containsKey(AuthenticationConstants.OAuth2.ERROR)) {
			...
			String correlationInResponse = response
					.get(AuthenticationConstants.AAD.CORRELATION_ID);
			if (!StringExtensions.IsNullOrBlank(correlationInResponse)) {
				try {
					correlationId = UUID.fromString(correlationInResponse);
				} catch (IllegalArgumentException ex) {
					correlationId = null;
					Logger.e(TAG, "CorrelationId is malformed: "
			...
			if (response.containsKey(AuthenticationConstants.OAuth2.ID_TOKEN)) {
				...
				String idToken = response
						.get(AuthenticationConstants.OAuth2.ID_TOKEN);
			...
			result = new AuthenticationResult(
					...
					response.get(AuthenticationConstants.OAuth2.REFRESH_TOKEN),
					expires.getTime(), isMultiResourcetoken, userinfo);
	...
	private static void extractJsonObjects(
			HashMap<String, String> responseItems, String jsonStr)
	...
	public void refreshToken(
		...
		if (mWebRequestHandler == null) {
			...
			authenticationCallback.onError(new IllegalArgumentException(
					"webRequestHandler"));
			return;
		}
		...
		try {
			...
			Logger.e(TAG, encoding.getMessage(), "",
		...
		try {
			...
			mWebRequestHandler.sendAsyncPost(authority, headers, requestMessage
					...
					new HttpWebRequestCallback() {
						...
						public void onComplete(HttpWebResponse response,
								Exception exception) {
							if (exception != null) {
								Logger.e(TAG, "Web request returned exception"
										+ exception.getMessage(), "",
										ADALError.SERVER_ERROR, exception);
								authenticationCallback.onError(exception);
							} else {
								Logger.v(TAG,
										"Token request does not have errors");
	 ...
	 *            browser reached to this final url and it has code or token for
	 *            next step
			...
			if (!StringExtensions.IsNullOrBlank(authorizationUri)
				...
				if (result.getStatus() == AuthenticationResult.AuthenticationStatus.Succeeded) {
					if (!result.getCode().isEmpty()) {
						...
						getTokenForCode(result.getCode(),
								authenticationCallback);

					} else if (!StringExtensions.IsNullOrBlank(result
							.getAccessToken())) {
					...
					authenticationCallback
							.onError(new AuthenticationException(result
									.getErrorCode(), result
									.getErrorDescription()));
				}
	...
	private HashMap<String, String> getUrlParameters(String finalUrl) {
		...
		HashMap<String, String> parameters = HashMapExtensions
				.URLFormDecode(fragment);
	...
	public String encodeProtocolState() {
		String state = String.format("a=%s&r=%s", mRequest.getAuthority(),
		...
		if (webResponse.getBody() != null && webResponse.getBody().length > 0) {
			...
			try {
				...
				Logger.e(TAG, ex.getMessage(), "",
						ADALError.SERVER_INVALID_JSON_RESPONSE, ex);
				result = new AuthenticationResult(JSON_PARSING_ERROR,
						ex.getMessage(), null);
			}
			...
			if (message != null) {
				...
				errMessage = "Status code:"
						+ String.valueOf(webResponse.getStatusCode());
			}
			Logger.v(TAG, "Server error message:" + errMessage);
			result = new AuthenticationResult(String.valueOf(webResponse
					.getStatusCode()), errMessage, null);
		}
