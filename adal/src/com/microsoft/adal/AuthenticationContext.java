...
public class AuthenticationContext {
	 ...
	 * 
	 * @param appContext
	 *            It needs to have handle to the context to use the
	 *            SharedPreferences as a Default cache storage. It does not need
	 *            to be activity.
	 * @param authority
	 *            Authority url to send code and token requests
	 * @param validateAuthority
	 *            validate authority before sending token request
	 */
	 ...
	 * @param validateAuthority
	 * @param cache
	 *            Set to null if you don't want cache.
	 */
	public AuthenticationContext(Context appContext, String authority,
	 ...
	 * 
	 * @param activity
	 *            required to launch authentication activity.
	 * @param resource
	 *            required resource identifier.
	 * @param clientId
	 *            required client identifier
	 * @param redirectUri
	 *            Optional. It will use package name info if not provided.
	 * @param userId
	 *            Optional.
	 * @param callback
	 *            required
	 */
	public void acquireToken(Activity activity, String resource,
			String clientId, String redirectUri, String userId,
			AuthenticationCallback<AuthenticationResult> callback) {

		redirectUri = checkInputParameters(activity, resource, clientId,
				redirectUri, callback);

		final AuthenticationRequest request = new AuthenticationRequest(
				mAuthority, resource, clientId, redirectUri, userId,
	 ...
	 * @param clientId
	 * @param redirectUri
	 *            Optional. It will use packagename and provided suffix for
	 *            this.
	 * @param userId
	 *            Optional. This parameter will be used to pre-populate the
	 ...
	 *            Optional. This parameter will be appended as is to the query
	 *            string in the HTTP authentication request to the authority.
	...
	public void acquireToken(Activity activity, String resource,
		...
		final AuthenticationRequest request = new AuthenticationRequest(
				mAuthority, resource, clientId, redirectUri, userId,
				PromptBehavior.Auto, extraQueryParameters);
	 ...
	 *            this.
	 * @param prompt
	 *            Optional. added as query parameter to authorization url
	 * @param callback
	 */
	public void acquireToken(Activity activity, String resource,
		...
		final AuthenticationRequest request = new AuthenticationRequest(
				mAuthority, resource, clientId, redirectUri, null, prompt, null);
	...
	public void acquireToken(Activity activity, String resource,
		...
		final AuthenticationRequest request = new AuthenticationRequest(
				mAuthority, resource, clientId, redirectUri, null, prompt,
	 ...
	 *            this.
	 * @param userId
	 *            Optional. It is used for cache and as a loginhint at
	 *            authentication.
	 * @param prompt
	 *            Optional. added as query parameter to authorization url
	 * @param extraQueryParameters
	...
	public void acquireToken(Activity activity, String resource,
		...
		final AuthenticationRequest request = new AuthenticationRequest(
				mAuthority, resource, clientId, redirectUri, userId, prompt,
				extraQueryParameters);

		acquireTokenLocal(activity, request, callback);
	}

	private String checkInputParameters(Activity activity, String resource,
		...
		if (requestCode == AuthenticationConstants.UIRequest.BROWSER_FLOW) {
			if (data == null) {
				...
				Logger.e(TAG, "onActivityResult BROWSER_FLOW data is null",
						null, ADALError.ON_ACTIVITY_RESULT_INTENT_NULL);
			} else {
				Bundle extras = data.getExtras();
				final int requestId = extras
						.getInt(AuthenticationConstants.Browser.REQUEST_ID);
				...
				if (resultCode == AuthenticationConstants.UIResponse.BROWSER_CODE_CANCEL) {
					...
					Logger.v(TAG, "Error code from webview:" + errCode
							+ " for requestId: " + requestId);
					waitingRequestOnError(waitingRequest, requestId,
							new AuthenticationException(
									ADALError.SERVER_INVALID_REQUEST, errCode
											+ " " + errMessage));
					...
					if (endingUrl.isEmpty()) {
						...
						Oauth2 oauthRequest = new Oauth2(authenticationRequest,
								mWebRequest);
								...
								.getToken(
										...
										new AuthenticationCallback<AuthenticationResult>() {
											...
											public void onSuccess(
												...
												Logger.v(TAG,
														"Token is received and setting item to cache");
												setItemToCache(
	...
	private AuthenticationRequestState getWaitingRequest(int requestId) {
		...
		if (request == null && mAuthorizationCallback != null
			...
			// callback if set
			Logger.e(TAG, "Request callback is not available for requestid:"
					+ requestId + ". It will use last callback.", "",
					ADALError.CALLBACK_IS_NOT_FOUND);
			request = new AuthenticationRequestState(0, null,
	...
	private void putWaitingRequest(int requestId,
			AuthenticationRequestState requestState) {
	...
	public boolean cancelAuthenticationActivity(int requestId) {
		...
		if (request == null || request.mDelagete == null) {
			...
			Logger.v(TAG,
					"Current callback is empty. There is not any active authentication.");
			return true;
		}

		Logger.v(TAG,
				"Current callback is not empty. There is an active authentication Activity.");

		// intent to cancel. Authentication activity registers for this message
		// at onCreate event.
		final Intent intent = new Intent(
				AuthenticationConstants.Browser.ACTION_CANCEL);
		...
		boolean cancelResult = LocalBroadcastManager.getInstance(mContext)
	...
	private void acquireTokenLocal(final Activity activity,
		...
		if (mValidateAuthority && !mAuthorityValidated) {
			validateAuthority(authorityUrl,
					new AuthenticationCallback<Boolean>() {

						@Override
						public void onSuccess(Boolean result) {
							if (result) {
								mAuthorityValidated = true;
								Logger.v(TAG, "Authority is validated: "
										+ authorityUrl.toString());
								acquireTokenAfterValidation(activity, request,
										externalCall);
							} else {
								Logger.v(TAG,
										"Call external callback since instance is invalid"
												+ authorityUrl.toString());
								externalCall
						...
						public void onError(Exception exc) {
							Logger.e(
									TAG,
									"Authority validation has an error",
									"",
									ADALError.DEVELOPER_AUTHORITY_IS_NOT_VALID_INSTANCE,
									exc);
							externalCall
	...
	private void acquireTokenAfterValidation(final Activity activity,
		...
		if (request.getPrompt() != PromptBehavior.Always && refreshItem != null
			...
			Logger.v(TAG,
					"Refresh token is available and it will attempt to refresh token");
			...
			if (request.getPrompt() != PromptBehavior.Never) {
				...
				Logger.v(TAG, "Starting Authentication Activity with callback:"
						+ externalCall.hashCode());
				putWaitingRequest(externalCall.hashCode(),
						new AuthenticationRequestState(externalCall.hashCode(),
	...
	protected boolean isRefreshable(AuthenticationResult cachedItem) {
		return cachedItem != null
				&& !StringExtensions
						.IsNullOrBlank(cachedItem.getRefreshToken());
	}

	private boolean isValidCache(AuthenticationResult cachedItem) {
		if (cachedItem != null
				&& !StringExtensions.IsNullOrBlank(cachedItem.getAccessToken())
	...
	private RefreshItem getRefreshToken(final AuthenticationRequest request) {
		...
		if (mTokenCacheStore != null) {
			...
			if (item == null
					|| StringExtensions.IsNullOrBlank(item.getRefreshToken())) {
	...
	private void setItemToCache(final AuthenticationRequest request,
			AuthenticationResult result) throws AuthenticationException {
		if (mTokenCacheStore != null) {
			// Store token
			Logger.v(TAG, "Setting item to cache");
			mTokenCacheStore.setItem(CacheKey.createCacheKey(request),
			...
			if (result.getIsMultiResourceRefreshToken()) {
				...
				mTokenCacheStore.setItem(
						CacheKey.createMultiResourceRefreshTokenKey(request),
	 ...
	 * 
	 * @param activity
	 *            Activity to use in case refresh token does not succeed and
	 *            prompt is not set to never.
	 * @param request
	 *            incoming request
	 * @param refreshToken
	 *            refresh token
	 * @param prompt
	 *            if set to never, it should not attempt to launch authorization
				...
				new AuthenticationCallback<AuthenticationResult>() {
					...
					public void onSuccess(AuthenticationResult result) {
						...
						if (result == null
							...
							Logger.w(
									TAG,
									"Refresh token did not return access token",
									request.getLogInfo(),
							...
							if (useCache) {
								...
								Logger.v(TAG,
										"Cache is used. It will remove this item from cache and try again."
												+ request.getLogInfo());
								removeItemFromCache(refreshItem);
								acquireTokenLocal(activity, request,
										externalCallback);
							} else {
								...
								Logger.w(
										TAG,
										"Cache is not used. It wil return error to user",
										request.getLogInfo(),
										ADALError.AUTH_REFRESH_FAILED_PROMPT_NOT_ALLOWED);
								externalCallback
										.onError(new AuthenticationException(
							...
							Logger.v(TAG, "Refresh token is finished for "
									+ request.getLogInfo());
							if (useCache) {
					...
					public void onError(Exception exc) {
						...
						Logger.e(TAG, "Error in refresh token for request:"
								+ request.getLogInfo(),
		...
		if (mDiscovery != null) {
			...
			mDiscovery.isValidAuthority(authorityUrl,
					new AuthenticationCallback<Boolean>() {
						...
						public void onSuccess(Boolean result) {
							Logger.v(TAG,
									"Instance validation is successfull. Result:"
											+ result.toString());
	...
	final private Intent getAuthenticationActivityIntent(Activity activity,
		...
		intent.putExtra(AuthenticationConstants.Browser.REQUEST_MESSAGE,
				request);
		return intent;
	}
	 ...
	 */
	private void refreshTokenWithoutCache(final String refreshToken,
			String clientId, String resource,
		...
		if (mValidateAuthority) {
			...
			validateAuthority(authorityUrl,
					new AuthenticationCallback<Boolean>() {
							...
							if (result) {
								...
								Logger.v(TAG,
										"Authority is validated"
												+ authorityUrl.toString());
								refreshToken(null, request, refreshItem, false,
										callback);
							} else {
								Logger.v(TAG,
						...
						@Override
						public void onError(Exception exc) {
							Logger.e(TAG, "Authority validation is failed", ExceptionExtensions.getExceptionMessage(exc), ADALError.SERVER_INVALID_REQUEST, exc);
